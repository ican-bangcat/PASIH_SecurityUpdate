      ...
      <form method="POST" action="{{ route('admin.accounts.update', $account) }}" class="p-4 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
            <div class="mt-2 text-xs grid grid-cols-2 gap-2 text-slate-500" id="passwordRules">
        ...
        <div class="pt-1">
          <button type="submit" id="submitBtn" class="inline-flex items-center gap-2 h-10 px-4 rounded-md bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 transition-all">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function updateValidation() {
        ...
        // If password is empty in the edit page, it is optional, so it's valid.
        if (password.length === 0) {
            ...
            for (const key in rules) {
                ...
                rule.element.classList.remove('text-rose-500', 'text-emerald-600');
