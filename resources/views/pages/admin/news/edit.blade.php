...
@section('content')
  ...
  <div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="pasih-page-title">Edit Berita</h1>
    ...
    <div class="rounded-xl bg-white ring-1 ring-slate-200 overflow-hidden shadow-xs">
      <div class="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
        <h2 class="text-[18px] font-bold text-slate-800">Ubah Data Berita</h2>
        <a href="{{ route('admin.news.show', $news) }}" class="text-xs text-blue-900 font-semibold hover:underline">
          Lihat Pratinjau &rarr;
        </a>
      </div>

      <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data" class="p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              type="text"
              name="author_name"
              value="{{ old('author_name', $news->author_name ?? $news->author?->name ?? 'Humas Kanwil Kemenkumham Riau') }}"
          ...
          <div>
            ...
            >
              <option value="published" @selected(old('status', $news->status) === 'published')>Publikasikan</option>
          ...
          <div>
            ...
            <input
              type="date"
              id="published_date_edit"
              name="published_date"
              value="{{ old('published_date', optional($news->published_at)->format('Y-m-d') ?? now()->format('Y-m-d')) }}"
          ...
          <div>
            ...
            <div class="relative mt-2">
              <input
                type="text"
                id="published_time_edit"
                name="published_time"
                value="{{ old('published_time', optional($news->published_at)->format('H:i') ?? now()->format('H:i')) }}"
                readonly
          ...
          <div class="md:col-span-2">
            ...
            <p class="mt-1 text-xs text-slate-500">
              Biarkan kosong jika tidak ingin mengubah gambar sampul yang sudah ada.
            </p>

            @if($news->image_url)
              <div class="my-3 flex items-center gap-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
                <img src="{{ $news->image_url }}" alt="Sampul Saat Ini" class="w-24 h-16 object-cover rounded-md border border-slate-200 shadow-xs">
                <div>
                  <p class="text-xs font-semibold text-slate-700">Gambar Sampul Saat Ini</p>
                  <p class="text-[11px] text-slate-500 truncate max-w-xs">{{ $news->image_path }}</p>
                </div>
              </div>
            @endif
            ...
            <div id="image-preview-container-edit" class="mt-3 hidden">
              <p class="text-xs font-semibold text-slate-600 mb-1">Pratinjau Gambar Pengganti Baru:</p>
              <img id="image-preview-edit" src="#" alt="Preview Pengganti" class="max-h-48 rounded-lg object-cover border border-slate-200 shadow-xs">
            </div>
            @error('image')
              <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
            @enderror
  ...
  <script>
    ...
    mdtimepicker('#published_time_edit', {
    ...
    document.addEventListener('click', function(e) {
      if (e.target && e.target.id === 'published_time_edit') {
        setTimeout(function() {
    ...
    // Image Preview
    const imageInput = document.getElementById('news-image-input-edit');
