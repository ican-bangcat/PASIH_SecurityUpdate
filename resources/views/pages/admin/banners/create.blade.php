@extends('layouts.app')
@section('title', 'Tambah Foto Banner Beranda')
...
@section('content')
  <div class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="pasih-page-title">Tambah Foto Banner Beranda</h1>
    ...
    <div class="rounded-xl bg-white ring-1 ring-slate-200 overflow-hidden shadow-xs">
      <div class="px-4 py-3 border-b border-slate-200">
        <h2 class="text-[18px] font-bold text-slate-800">Formulir Foto Banner Kegiatan</h2>
      </div>

      <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div class="md:col-span-2">
            <label class="block text-sm font-semibold text-slate-700">
              Foto Banner Kegiatan <span class="text-rose-500">*</span>
            </label>
            <p class="mt-1 text-xs text-slate-500">
              Format: JPG, JPEG, PNG, atau WEBP. Maksimal ukuran file 5 MB (Rekomendasi rasio 16:9 atau 4:3 landscape agar simetris di landing page).
            </p>
            <input
              type="file"
              name="image"
            ...
            <div id="image-preview-container" class="mt-3 hidden">
              <p class="text-xs font-semibold text-slate-600 mb-1">Pratinjau Foto Banner:</p>
              <img id="image-preview" src="#" alt="Preview Banner" class="max-h-56 rounded-xl object-cover border border-slate-200 shadow-xs">
            </div>
            @error('image')
          ...
          <div class="md:col-span-2">
            <label class="block text-sm font-semibold text-slate-700">
              Judul / Keterangan Foto Kegiatan (Opsional)
            </label>
            <input
              type="text"
              name="title"
              value="{{ old('title') }}"
              placeholder="Contoh: Rapat Pleno Pengharmonisasian Raperda se-Provinsi Riau..."
          ...
          <div>
            ...
            >
            <p class="mt-1 text-xs text-slate-500">Angka lebih kecil tampil lebih awal (contoh: 1, 2, 3...).</p>
            @error('order')
              <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            ...
            >
              <option value="1" @selected(old('is_active', '1') == '1')>Aktif (Tampilkan di Beranda)</option>
              <option value="0" @selected(old('is_active') == '0')>Nonaktif (Sembunyikan Sementara)</option>
            </select>
            @error('is_active')
              <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
            @enderror
          ...
          <div class="md:col-span-2">
            ...
            <textarea
              name="description"
              rows="3"
              placeholder="Catatan internal mengenai dokumentasi kegiatan ini..."
